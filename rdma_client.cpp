...
static int sock_connect(const char* remote_ip, int port) {
    ...
    // 获取地址信息
    int ret = getaddrinfo(remote_ip, std::to_string(port).c_str(), &hints, &resolved_addr);
    if (ret != 0) {
        fprintf(stderr, "getaddrinfo: %s for port %d\n", gai_strerror(ret), port);
        return -1;
    }
    ...
    for (iterator = resolved_addr; iterator != NULL; iterator = iterator->ai_next) {
        ...
        fprintf(stdout, "sock_connect: created socket fd = %d\n", sockfd);
        if (sockfd >= 0) {
    ...
    freeaddrinfo(resolved_addr); // 释放地址信息

    return sockfd; // 成功时返回 sockfd，失败时返回 -1
}
...
static int connect_qp(struct resources *res) {
	...
	fprintf(stdout, "本地sock=%d", res->sock);

    sock_sync_data(res->sock, 5, "world", (char *)&tmp_con_data);
...
static int post_send(struct resources *res, ibv_wr_opcode opcode, char* msg)
{
...
int main() {
	...
	// if (post_send(&res, IBV_WR_SEND, "server message"))
	// {
	// 	fprintf(stderr, "failed to post sr\n");
	// 	goto main_exit;
	// }
